...
example (E : Set X) : IsCompact E
  <-> ∀ G : OpenCover E, ∃ G' : SubCover G, IsFiniteCover G'.val := by
    ...
    · intro hG
      ...
      intro ι U hUo hsU
      have hG' : ∀ G : OpenCover.{u_1} E, ∃ G' : SubCover G, IsFiniteCover G'.val := hG
      let G : OpenCover.{u_1} E :=
        { ι := ι
          U := U
          isOpen := hUo
          subset_iUnion := hsU }
      rcases hG' G with ⟨G', _⟩
      have hcover : E ⊆ ⋃ a : G'.ι, G.U (G'.idx a) := G'.covers
      refine ⟨Finset.univ.map G'.idx, ?_⟩
      intro x hx
      rcases hcover hx with ⟨a, ha⟩
      refine ⟨G'.idx a, ?_, ha⟩
      have : a ∈ (Finset.univ : Finset G'.ι) := Finset.mem_univ _
      exact Finset.mem_map.mpr ⟨a, this, rfl⟩
